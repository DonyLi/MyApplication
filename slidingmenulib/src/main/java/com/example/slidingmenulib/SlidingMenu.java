...
public class SlidingMenu extends FrameLayout {
    ...
    private void init() {
        ...
        this.mMenuContainer.setId(R.id.f1);
        this.mMainContainer = new FrameLayout(getContext());
        this.mMainContainer.setId(R.id.f2);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case 0:
                ...
                if ((this.mCurState == 2) && (ev.getX() > -this.mMainContainer.getScrollX() * this.mMenuWidth / ((1.0F -
                        (1.0F - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth))) {
                    return true;
                }
                break;
            case 2:
        ...
        switch (event.getAction()) {
            ...
            case 1:
                if ((this.mMaxMoveX < this.mTouchSlop) && (event.getX() >= this.mMenuWidth)) {
                    ...
                    this.mMaxMoveX = 0.0F;
                    if (ViewHelper.getScrollX(this.mMainContainer) != -(int) ((1.0F - (1.0F - this.mMenuRatio) / this
                            .mMainMinScale) * this.mViewWidth)) {
                        if (ViewHelper.getScrollX(this.mMainContainer) >= -(int) ((1.0F - (1.0F - this.mMenuRatio) / this
                                .mMainMinScale) * this.mViewWidth) / 2)
                            closeMenu();
                        else
                            openMenu();
                    }
    ...
    private void handleScroll(int mainScrollX) {
        ...
        ViewHelper.setScaleX(this.mMainContainer, 1.0F - (1.0F - this.mMainMinScale) * mainScrollX / -(int) ((1.0F - (1.0F
                - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth));
        ViewHelper.setScaleY(this.mMainContainer, 1.0F - (1.0F - this.mMainMinScale) * mainScrollX / -(int) ((1.0F - (1.0F
                - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth));

        float menuScrollX = this.mMenuWidth * this.mMenuPadingRatio - this.mMenuWidth * mainScrollX / (1.0F / this
                .mMenuPadingRatio * -(1.0F - (1.0F - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth);
        ...
        ViewHelper.setScaleX(this.mMenuContainer, this.mMenuMinScale - mainScrollX * (1.0F - this.mMenuMinScale) / ((1.0F
                - (1.0F - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth));
        ViewHelper.setScaleY(this.mMenuContainer, this.mMenuMinScale - mainScrollX * (1.0F - this.mMenuMinScale) / ((1.0F
                - (1.0F - this.mMenuRatio) / this.mMainMinScale) * this.mViewWidth));
    }
